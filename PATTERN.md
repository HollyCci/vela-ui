...
本项目**自研**一套类 HeroUI Pro 的组件库（无 Pro 授权，不能直接装真包）。CSS 已就绪
（`src/styles/components/<块名>.css`，构建时全量引入），**不要写新的 CSS**（展示站布局除外）。

**核心原则：渲染对的 BEM 类名只是「长得像」，不等于「是同一个组件」。** 目标是行为 1:1，
不是 DOM 1:1。真站每个组件都是某个最佳引擎的薄封装——必须接入**同款真引擎**，而不是用基础原语近似：

| 能力 | 真站引擎（必须用） | 反面教材（不要再做） |
| --- | --- | --- |
